...
void main() {
  test('Stream awaiter test', () async {
    ...
    // Cancel at value 7
    expect(stream.awaitValue(8, cancelValue: 7, ct: ct1), completion(isFalse));
    ...
    // Cancel at value 1
    expect(stream.awaitValue(8, cancelValue: 1, ct: ct2), completion(isFalse));

    // Cancel with ct2 (just before 5)
    expect(stream.awaitValue(5, ct: ct2), completion(isFalse));

    // Cancel with ct2 (just before 5)
    expect(stream.awaitValue(5, cancelValue: 8, ct: ct2), completion(isFalse));

    // Cancel with ct2 (just before 5)
    expect(stream.awaitValue(8, cancelValue: 7, ct: ct2), completion(isFalse));
